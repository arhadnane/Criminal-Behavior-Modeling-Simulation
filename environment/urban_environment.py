...
class UrbanEnvironment:
    ...
    def get_lighting(self, x, y):
        """Get lighting value at specific coordinates"""
        if 0 <= x < self.width and 0 <= y < self.height:
            return self.lighting_grid[x][y]
        return 0.0
    
    def get_surveillance(self, x, y):
        """Get surveillance coverage at specific coordinates"""
        if 0 <= x < self.width and 0 <= y < self.height:
            return self.surveillance_grid[x][y]
        return 0.0
    
    def get_socioeconomic_status(self, x, y):
        """Get socioeconomic status at specific coordinates"""
        if 0 <= x < self.width and 0 <= y < self.height:
            return self.socioeconomic_grid[x][y]
        return 0.0
    
    def calculate_risk(self, x, y):
        """Calculate crime risk at specific location"""
        if not (0 <= x < self.width and 0 <= y < self.height):
            return 0.0
        
        lighting = self.get_lighting(x, y)
        surveillance = self.get_surveillance(x, y)
        socioeconomic = self.get_socioeconomic_status(x, y)
        police_presence = self.police_presence_grid[x][y]
        
        # Higher risk with poor lighting, low surveillance, low socioeconomic status, low police presence
        risk = (
            (1.0 - lighting) * 0.3 +
            (1.0 - surveillance) * 0.3 +
            (1.0 - socioeconomic) * 0.2 +
            (1.0 - police_presence) * 0.2
        )
        
        # Apply temporal factors
        temporal_factor = 1.0
        if hasattr(self.model, 'current_hour'):
            # Higher risk during night hours
            if 22 <= self.model.current_hour or self.model.current_hour <= 5:
                temporal_factor = 1.4
            elif 18 <= self.model.current_hour <= 21:
                temporal_factor = 1.2
        
        return min(risk * temporal_factor, 1.0)
    
    def calculate_attractiveness(self, x, y):
        """Calculate location attractiveness for criminal activity"""
        if not (0 <= x < self.width and 0 <= y < self.height):
            return 0.0
        
        # Attractiveness factors
        population = self.population_density_grid[x][y]
        lighting = self.get_lighting(x, y)
        surveillance = self.get_surveillance(x, y)
        police = self.police_presence_grid[x][y]
        
        # High population density increases attractiveness (more targets)
        # Low lighting and surveillance increase attractiveness (easier to commit crimes)
        # Low police presence increases attractiveness
        attractiveness = (
            population * 0.4 +  # More people = more opportunities
            (1.0 - lighting) * 0.2 +  # Darkness helps criminals
            (1.0 - surveillance) * 0.2 +  # Less watching eyes
            (1.0 - police) * 0.2  # Less law enforcement
        )
        
        return min(attractiveness, 1.0)
    
    def update_time_based_factors(self, hour):
        """Update environmental factors based on time of day"""
        # Store current hour in model for other calculations
        self.model.current_hour = hour
        ...
        for x in range(self.width):
            for y in range(self.height):
                base_lighting = self.lighting_grid[x][y]
                # Adjust lighting but maintain some variation based on infrastructure
                self.lighting_grid[x][y] = base_lighting * lighting_factor
        
        # Update police presence (more at night in some areas, less in others)
        for x in range(self.width):
            for y in range(self.height):
                base_police = self.police_presence_grid[x][y]
                if 22 <= hour or hour <= 5:  # Night shift
                    # Increase police in high-crime areas, decrease in safe areas
                    crime_history = 0.5  # Simplified, would use actual crime data
                    if crime_history > 0.6:
                        self.police_presence_grid[x][y] = min(base_police * 1.3, 1.0)
                    else:
                        self.police_presence_grid[x][y] = base_police * 0.8
                else:  # Day shift
                    self.police_presence_grid[x][y] = base_police
